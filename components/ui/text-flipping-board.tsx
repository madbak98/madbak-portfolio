...
export type TextFlippingBoardTheme = "default" | "madbak";
...
const MADBAK_ACCENT_COLORS: AccentColor[] = [
  { top: "bg-[#ff2a2a]", bottom: "bg-[#b91c1c]", text: "text-white" },
  { top: "bg-[#ebe8e1]", bottom: "bg-[#c9c5bd]", text: "text-[#0a0a0a]" },
];
...
const FlapCell = React.memo(function FlapCell({
  ...
  useEffect(() => {
    ...
    const runStep = (i: number) => {
      ...
      const accentColors = theme === "madbak" ? MADBAK_ACCENT_COLORS : ACCENT_COLORS;
  ...
  }, [target, delay, stepMs, theme, reducedMotion]);

  const normalizedTarget = FLAP_CHARS.includes(target.toUpperCase())
    ? target.toUpperCase()
    : " ";
  const visibleCurrent = reducedMotion || current === " " ? normalizedTarget : current;
  const visiblePrev = reducedMotion || prev === " " ? normalizedTarget : prev;
  const visibleAccent = reducedMotion ? null : accent;
  const visiblePrevAccent = reducedMotion ? null : prevAccent;
  const show = visibleCurrent === " " ? "\u00A0" : visibleCurrent;
  const showPrev = visiblePrev === " " ? "\u00A0" : visiblePrev;

  const textCx =
    "absolute inset-x-0 flex select-none items-center justify-center font-mono font-bold tracking-wide";
  const topBg =
    visibleAccent?.top ??
    (theme === "madbak" ? "bg-[#171717]" : "bg-neutral-200/80 dark:bg-neutral-900");
  const bottomBg =
    visibleAccent?.bottom ??
    (theme === "madbak" ? "bg-[#101010]" : "bg-neutral-200/80 dark:bg-neutral-900");
  const textColor =
    visibleAccent?.text ??
    (theme === "madbak" ? "text-[#ebe8e1]" : "text-neutral-800 dark:text-white");

  const flapTopBg =
    visiblePrevAccent?.top ??
    (theme === "madbak" ? "bg-[#242424]" : "bg-neutral-100 dark:bg-neutral-800");
  const flapTextColor =
    visiblePrevAccent?.text ??
    (theme === "madbak" ? "text-[#ebe8e1]" : "text-neutral-800 dark:text-white");
  ...
  return (
    <div
      className={cn(
        "flex aspect-3/6 flex-col overflow-hidden rounded-[2px] md:rounded-[3px] md:border-2",
        theme === "madbak"
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        theme === "madbak"
          ? "relative mx-auto w-full max-w-3xl rounded-[3px] bg-[#0a0a0a] p-2 shadow-[0_24px_80px_rgba(0,0,0,0.42)] md:p-4"
